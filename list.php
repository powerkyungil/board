    ...
    <div class="container">
      <div id="board_area">
        ...
        <table class="table table-striped" style="text-align: center; border: 1px solid #ddddda">
          ...
          <?php
            $sql = dbConnect("SELECT * FROM board");
            ...
            $boardSelect = dbConnect("SELECT * FROM board ORDER BY idx DESC LIMIT $page_start, $list");

            while ($board = $boardSelect->fetch_array()) {
              $title = $board["title"];
              if(strlen($title)>30) {
                ...
                <td width="500">
                  ...
                  <?php
                    ...
                    if($board['lock_post'] == "1") {
                        //lock_post 값이 1이면 잠금
                  ?>
                    <span class="lock_check" style="cursor:pointer" data-action="./read.php?idx=" data-idx="<?=$board['idx']?>"
                          data-check="<?=$role?>" >
                      <?=$title?><?=$lockimg?>
                    </span>
                  <?php
                  ...
                  ?>
                    <span class="read_check" style="cursor:pointer" data-action="./read.php?idx=<?=$board['idx']?>">
                        <?=$title?>
            ...
            <?php
                ...
                for($i = $block_start; $i <= $block_end; $i++) {
                    if($page == $i) {
                        ...
                        echo "<a href='list.php?page=$i' style='font-size: 20px;'>$i</a>";
                    }
                }
